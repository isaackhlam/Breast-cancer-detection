...
pre_test = dataset.CIFAR10(
        root = "./cifar",
        transform = pre_transforms,
        train = False,
)

data_loader_pre_train = torch.utils.data.DataLoader(
        dataset = pre_train,
        batch_size = 128,
        shuffle = True
)

data_loader_pre_test = torch.utils.data.DataLoader(
        dataset = pre_test,
        batch_size = 128,
        shuffle = False
)
...
name = "github-version-10"

wandb.login(key = os.getenv('WANDB_API_KEY'))
wandb.init(
    project = "Breast-Cancer-Image-Diganosis",
    name = name,
    notes = "Full mammogram image only, with single CNN layer, spliting train data to validate, data Aug, pretrain on CIFAR10",
