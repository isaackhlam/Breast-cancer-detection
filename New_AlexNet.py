...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.conv = torch.nn.Sequential(
            torch.nn.Conv2d(3, 96, kernel_size = 11, stride = 4, padding = 2), # 96 * 55 * 55
            torch.nn.BatchNorm2d(96),
            torch.nn.ELU(),
            torch.nn.Conv2d(96, 256, kernel_size = 5, stride = 1, padding = 2), # 256 * 55 * 55
            torch.nn.ELU(),
            torch.nn.BatchNorm2d(256),
            torch.nn.Conv2d(256, 384, kernel_size = 3, stride = 1, padding = 1), # 384 * 55 * 55
            torch.nn.ELU(),
            torch.nn.Conv2d(384, 384, kernel_size = 3, stride = 1, padding = 1), # 384 * 55 * 55
            torch.nn.ELU(),
            torch.nn.Conv2d(384, 256, kernel_size = 3, stride = 1, padding = 1), # 256 * 55 * 55
            torch.nn.ELU(),
            torch.nn.BatchNorm2d(256),
        )

        self.dense = torch.nn.Sequential(
            torch.nn.Linear(256 * 55 * 55, 4096),
...
print("Train set:" + len(train_loader.sampler.indices))
print("Valid set:" + len(valid_loader.sampler.indices))
print("Test set: " + len(test_set))

name = "AlexNet-No-Pool"
