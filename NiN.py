...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.conv = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(3, 64, kernel_size = 11, stride = 4, padding = 0),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(64, 64, kernel_size = 1, stride = 1, padding = 1),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(64, 64, kernel_size = 1, stride = 1, padding = 1),
            torch.nn.ReLU(inplace=True),
            torch.nn.MaxPool2d(3, 2, 0),
            # Feature extration 2
            torch.nn.Conv2d(64, 192, kernel_size = 5, stride = 1, padding = 2),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(192, 192, kernel_size = 1, stride = 1, padding = 1),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(192, 192, kernel_size = 1, stride = 1, padding = 1),
            torch.nn.ReLU(inplace=True),
            torch.nn.MaxPool2d(3, 2, 0),
            # Feature extration 3
            torch.nn.Conv2d(192, 2, kernel_size = 3, stride = 1, padding = 1),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(2, 2, kernel_size = 1, stride = 1, padding = 1),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(2, 2, kernel_size = 1, stride = 1, padding = 1),
            torch.nn.ReLU(inplace=True),
...
name = "github-version-4"

wandb.login(key = os.getenv('WANDB_API_KEY'))
wandb.init(
    project = "Breast-Cancer-Image-Diganosis",
    name = name,
    notes = "Full mammogram image only, NiN, spliting train data to validate, no data aug",
    tags = ["baseline", "arXiv: 1312.4400"],
    config = {
        "learning rate": lr,
        "architecture": "NiN",
        "epochs": epoches,
