...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.conv = torch.nn.Sequential(
            torch.nn.Conv2d(3, 64, kernel_size = 3, stride = 1, padding = 1), # 64 * 224 * 224
            torch.nn.ReLU(),
            torch.nn.Conv2d(64, 64, kernel_size = 3, stride = 1, padding = 1), # 64 * 224 * 224
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(2, 2, 0), # 64 * 112 * 112
            torch.nn.Conv2d(64, 128, kernel_size = 3, stride = 1, padding = 1), # 128 * 112 * 112
            torch.nn.ReLU(),
            torch.nn.Conv2d(128, 128, kernel_size = 3, stride = 1, padding = 1), # 128 * 112 * 112
            torch.nn.ReLU(),
            ...
            torch.nn.Conv2d(512, 512, kernel_size = 3, stride = 1, padding = 1), # 512 * 14 * 14
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(2, 2, 0), # 512 * 7 * 7
        )
...
name = "github-version-3"

wandb.login(key = os.getenv('WANDB_API_KEY'))
wandb.init(
    project = "Breast-Cancer-Image-Diganosis",
    name = name,
    notes = "Full mammogram image only, VGG16, spliting train data to validate, no data aug",
    tags = ["baseline", "arXiv: 1409.1556"],
    config = {
        "learning rate": lr,
        "architecture": "VGG16",
        "epochs": epoches,
